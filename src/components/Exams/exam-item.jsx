...
export default function ExamItem({ exam }) {
  return (
    <div className='bg-gray-200 rounded-md py-4 px-3 mb-4'>
      <header className='flex items-center gap-3 mb-3'>
        <span className='text-4xl w-8 h-8 rounded-full'>
          <HiOutlinePuzzle />
        </span>
        <div>
          <h1 className='font-semibold'>{exam.titulo}</h1>
          <h3 className='text-gray-500 text-sm'>{exam.descripcion}</h3>
        </div>
      </header>
      <footer className='flex justify-around gap-3'>
        ...
        <span className='flex items-center bg-yellow-100 text-yellow-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-yellow-300 border border-yellow-300'>N° de preguntas: {exam.numeroDePreguntas}</span>
